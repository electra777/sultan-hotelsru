	...
	return (
		<div className={styles.product}>
			<div className={styles.imgWrapper}>
				<img src={data.image.url} alt={data.image.alt} className={styles.img} />
			</div>
			<div className={styles.colRight}>
				<div className={styles.inStock}>
					{data.inStock ? (
						<div className={styles.yes}>В наличии</div>
					) : (
						<div className={styles.no}>Нет в наличии</div>
					)}
				</div>
				...
				<div className={styles.featureShort}>
					...
					<div className={styles.brand}>
						Бренд:
						<span className={styles.value}>{data.brand}</span>
					</div>
					<div className={styles.vendorCode}>
						Артикул:
						<span className={styles.value}>{data.barcode}</span>
					</div>
				...
				<div className={styles.featureFull}>
					...
					<div className={styles.featureFullWrapper}>
						...
						<div className={styles.vendorCode}>
							Артикул:
							<span className={styles.value}>{data.barcode.slice(7)}</span>
						</div>
						<div className={styles.barcode}>
							Штрихкод:
							<span className={styles.value}>{data.barcode}</span>
						...
						<div className={styles.weight}>
							...
							{data.weight ? (
								<span className={styles.value}>
									{data.weight} <span className={styles.weightValue}>г</span>
								</span>
							) : (
								'-'
							)}
						...
						<div className={styles.volume}>
							...
							{data.volume ? (
								<span className={styles.value}>
									{data.volume}
									<span className={styles.volumeValue}>мл</span>
								</span>
							) : (
								' -'
							)}
